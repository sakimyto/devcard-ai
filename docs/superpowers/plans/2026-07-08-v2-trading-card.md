...
### Task 1: 検出拡張（coauthor.ts + コーパス fixture）

**Files:**
- Modify: `src/analyzers/coauthor.ts`
- Create: `tests/analyzers/__fixtures__/commit-corpus.json`
- Modify: `tests/analyzers/coauthor.test.ts`（describe 追加のみ、既存テストは変更しない）

**Interfaces:**
- Consumes: 既存 `isAiCommit(message: string, authorLogin: string | null): boolean`
- Produces: 同シグネチャ（拡張のみ）。既存テスト全パス = 旧実装オラクル担保

- [ ] **Step 1: コーパス fixture を作成**

`tests/analyzers/__fixtures__/commit-corpus.json`:

```json
[
  { "message": "feat: add parser\n\nCo-Authored-By: Claude <noreply@anthropic.com>", "authorLogin": null, "expected": true, "note": "existing: claude trailer" },
  { "message": "fix(ci): retry\n\n🤖 Generated with [Claude Code](https://claude.com/claude-code)\n\nCo-Authored-By: Claude <noreply@anthropic.com>", "authorLogin": null, "expected": true, "note": "existing: claude code footer" },
  { "message": "feat: dashboard\n\nGenerated with Cursor", "authorLogin": null, "expected": true, "note": "new: generated WITH (旧実装は generated BY のみ)" },
  { "message": "chore: bump deps", "authorLogin": "dependabot[bot]", "expected": true, "note": "existing: bot login" },
  { "message": "feat: onboarding flow\n\nCo-authored-by: Codex <codex@openai.com>", "authorLogin": null, "expected": true, "note": "new: codex trailer (旧実装の名前リストに codex が無い)" },
  { "message": "fix: null check\n\nCo-authored-by: Windsurf Agent <agent@codeium.com>", "authorLogin": null, "expected": true, "note": "existing: codeium" },
  { "message": "docs: update readme\n\nCo-authored-by: Gemini <gemini@google.com>", "authorLogin": null, "expected": true, "note": "new: gemini trailer" },
  { "message": "refactor: split module\n\nCo-authored-by: Cody <cody@sourcegraph.com>", "authorLogin": null, "expected": true, "note": "new: cody trailer" },
  { "message": "feat: search\n\nAssisted-by: GitHub Copilot", "authorLogin": null, "expected": true, "note": "new: Assisted-by trailer" },
  ...
  { "message": "perf: cache layer\n\nCommitted via Devin", "authorLogin": null, "expected": true, "note": "new: via <agent> footer" },
  { "message": "feat: initial commit", "authorLogin": "copilot-swe-agent[bot]", "expected": true, "note": "existing: [bot] suffix" }
]
```

- [ ] **Step 2: 失敗するコーパステストを追加**
...
Run: `bunx vitest run tests/analyzers/coauthor.test.ts`
Expected: FAIL — "generated WITH" / codex / gemini / cody / Assisted-by / via Devin の6ケースが落ちる（decoy と既存ケースは通る）

- [ ] **Step 4: coauthor.ts のパターンを拡張**

`src/analyzers/coauthor.ts` の3定数を以下に置換（他は変更しない）:

```typescript
const AI_COAUTHOR_PATTERNS = [
	/co-authored-by:\s.*\b(claude|copilot|cursor|aider|codeium|tabnine|amazon-q|codex|windsurf|cody|gemini|devin|sweep)\b/i,
	/co-authored-by:\s.*@(anthropic\.com|openai\.com|aider\.chat|sourcegraph\.com)/i,
	/co-authored-by:\s.*\d+\+[^@]+@users\.noreply\.github\.com/i,
	/co-authored-by:\s.*noreply@/i,
	/assisted-by:\s.*\b(claude|copilot|cursor|codex|gemini)\b/i,
];
...
const AI_MESSAGE_PATTERNS = [
	...
	/\[ai\]/i,
	/🤖/,
	/(committed|created|authored) via (claude|codex|copilot|cursor|devin|aider)\b/i,
];
...
- [ ] **Step 5: 全テストパスを確認（オラクル込み）**

Run: `bunx vitest run tests/analyzers/coauthor.test.ts && bun run test`
Expected: 全 PASS。既存テストが1つでも落ちたら**デグレ**なので expected を変えずパターン側を直す

- [ ] **Step 6: Commit**

```bash
git add src/analyzers/coauthor.ts tests/analyzers/coauthor.test.ts tests/analyzers/__fixtures__/commit-corpus.json
git commit -m "feat(detect): AI検出パターン拡張（generated with / codex / gemini / cody / assisted-by / via系）
...
- Produces: `handleRequest(params: RequestParams, graphql: GraphqlFn, now?: Date): Promise<HandlerResult>`。`HandlerResult = { svg: string; status: number; kind: 'ok' | 'not_found' | 'no_repos' | 'no_ai' | 'error' }`（不正ユーザー名の 400 は api 層の責務 — Task 9）。`RequestParams = { user: string; theme: string }`（modules パラメータは v2 で廃止）
...
- Modify: `src/ogp.ts`, `src/types/wasm.d.ts`, `wrangler.toml`, `tests/ogp.test.ts`, `api/index.ts`
...
describe('svgToPng font rendering regression', () => {
  it('renders text pixels (not blank) — font-missing detector', async () => {
    // 白背景に黒テキストだけの SVG。フォントが無いと全ピクセル白になる
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="200" height="60">
      <rect width="200" height="60" fill="#ffffff"/>
      <text x="10" y="40" font-size="32" fill="#000000" font-family="Inter">HELLO</text>
    </svg>`
    const png = await svgToPng(svg, 200)
    // PNG デコードはせず、resvg の render() ピクセルを直接検査するために
    // svgToPngWithPixels ではなく PNG バイト長ヒューリスティックを使う:
    // 全白 200x60 PNG は ~200bytes、テキスト描画ありは >600bytes
    expect(png.byteLength).toBeGreaterThan(600)
  })
})
```
...
Expected: FAIL — ogShare モジュール不在。フォントテストは現実装で byteLength が小さく FAIL（=現バグの再現）
