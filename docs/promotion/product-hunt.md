...
Drop in a username and you get a card: a rarity frame by tier (S animates a holo foil, right inside GitHub's image proxy), a 6-axis radar from a single 12-week window so it never contradicts itself, POWER calibrated so only the top tier crosses 9000, plus an element, an epithet (The Strategist, The Lone Wolf…), and traits that trigger from your real data — "Centurion — 215 AI-assisted commits in 12 weeks" appears on its own.
...
All pure SVG, deterministic, no LLM calls, 465 tests. The gallery on the homepage fills with everyone's cards as they summon.

Would love your take on the rubric — tier thresholds, the trait pool, tools I've missed.

## ギャラリー画像（4-5枚）

1. **tier-comparison.png** — S/A/B/D の4枚横並び（レアリティが一目で伝わる主力）✅ 作成済み
...
5. **（任意）holo.gif** — S ティアのホロアニメ録画（S ティアの実ユーザーが出てから）
