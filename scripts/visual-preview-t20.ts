...
const outDir = new URL('../.superpowers/sdd/visual-t20/', import.meta.url).pathname
...
// All 5 tiers × both themes (bumpy graph + 2 traits) — the full audit matrix.
for (const grade of ['S', 'A', 'B', 'C', 'D'] as Grade[]) {
  emit(`${grade}-dark`, { ...base, stats: { ...base.stats, grade } }, 'dark')
  emit(`${grade}-light`, { ...base, stats: { ...base.stats, grade } }, 'light')
}
...
// over9000: gold POWER + glow at the HP position (5-digit number vs the nameplate).
emit('S-dark-over9000', { ...base, stats: { ...base.stats, grade: 'S', power: 9420 } }, 'dark')

// Traits variants: 0 traits → flavor line; 1 trait; 2 traits (base). traits-0 also proves
// the right-side damage number is absent when no proof number exists.
...
// 39-char username boundary + over9000 gold POWER — the worst case for the name/POWER
// name-fit math (long login must clear the widest possible number at the HP position).
emit(
  'S-dark-39char',
  {
    ...base,
    ...
    stats: { ...base.stats, grade: 'S', power: 10200 },
  },
  'dark',
)
