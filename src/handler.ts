...
export async function handleRequest(params: RequestParams, graphql: GraphqlFn): Promise<HandlerResult> {
  const { user, modules, theme } = params

  if (!user) {
    return { svg: renderErrorCard('User parameter required', theme), status: 200 }
  }
  ...
  try {
    ...
    if (coauthor.aiCommits === 0 && tools.tools.length === 0) {
      return { svg: renderErrorCard('No AI activity detected yet', theme), status: 200 }
    }
    ...
    return { svg: renderErrorCard('Temporarily unavailable', theme), status: 200 }
  }
}
