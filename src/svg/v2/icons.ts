import { escapeXml } from '../utils'

// Brand icon path data — CC0 simple-icons, viewBox 24, copied verbatim (do not
// hand-edit brand geometry). Keyed by our toolId; the value is the icon slug's
// single <path d="…">. Tools without a brand path fall back to a rune below.
const BRAND_PATHS: Record<string, string> = {
  claude:
    'm4.7144 15.9555 4.7174-2.6471.079-.2307-.079-.1275h-.2307l-.7893-.0486-2.6956-.0729-2.3375-.0971-2.2646-.1214-.5707-.1215-.5343-.7042.0546-.3522.4797-.3218.686.0608 1.5179.1032 2.2767.1578 1.6514.0972 2.4468.255h.3886l.0546-.1579-.1336-.0971-.1032-.0972L6.973 9.8356l-2.55-1.6879-1.3356-.9714-.7225-.4918-.3643-.4614-.1578-1.0078.6557-.7225.8803.0607.2246.0607.8925.686 1.9064 1.4754 2.4893 1.8336.3643.3035.1457-.1032.0182-.0728-.164-.2733-1.3539-2.4467-1.445-2.4893-.6435-1.032-.17-.6194c-.0607-.255-.1032-.4674-.1032-.7285L6.287.1335 6.6997 0l.9957.1336.419.3642.6192 1.4147 1.0018 2.2282 1.5543 3.0296.4553.8985.2429.8318.091.255h.1579v-.1457l.1275-1.706.2368-2.0947.2307-2.6957.0789-.7589.3764-.9107.7468-.4918.5828.2793.4797.686-.0668.4433-.2853 1.8517-.5586 2.9021-.3643 1.9429h.2125l.2429-.2429.9835-1.3053 1.6514-2.0643.7286-.8196.85-.9046.5464-.4311h1.0321l.759 1.1293-.34 1.1657-1.0625 1.3478-.8804 1.1414-1.2628 1.7-.7893 1.36.0729.1093.1882-.0183 2.8535-.607 1.5421-.2794 1.8396-.3157.8318.3886.091.3946-.3278.8075-1.967.4857-2.3072.4614-3.4364.8136-.0425.0304.0486.0607 1.5482.1457.6618.0364h1.621l3.0175.2247.7892.522.4736.6376-.079.4857-1.2142.6193-1.6393-.3886-3.825-.9107-1.3113-.3279h-.1822v.1093l1.0929 1.0686 2.0035 1.8092 2.5075 2.3314.1275.5768-.3218.4554-.34-.0486-2.2039-1.6575-.85-.7468-1.9246-1.621h-.1275v.17l.4432.6496 2.3436 3.5214.1214 1.0807-.17.3521-.6071.2125-.6679-.1214-1.3721-1.9246L14.38 17.959l-1.1414-1.9428-.1397.079-.674 7.2552-.3156.3703-.7286.2793-.6071-.4614-.3218-.7468.3218-1.4753.3886-1.9246.3157-1.53.2853-1.9004.17-.6314-.0121-.0425-.1397.0182-1.4328 1.9672-2.1796 2.9446-1.7243 1.8456-.4128.164-.7164-.3704.0667-.6618.4008-.5889 2.386-3.0357 1.4389-1.882.929-1.0868-.0062-.1579h-.0546l-6.3385 4.1164-1.1293.1457-.4857-.4554.0608-.7467.2307-.2429 1.9064-1.3114Z',
  // toolId 'codex' → OpenAI mark (Codex is delivered via OpenAI models)
  codex:
    'M22.2819 9.8211a5.9847 5.9847 0 0 0-.5157-4.9108 6.0462 6.0462 0 0 0-6.5098-2.9A6.0651 6.0651 0 0 0 4.9807 4.1818a5.9847 5.9847 0 0 0-3.9977 2.9 6.0462 6.0462 0 0 0 .7427 7.0966 5.98 5.98 0 0 0 .511 4.9107 6.051 6.051 0 0 0 6.5146 2.9001A5.9847 5.9847 0 0 0 13.2599 24a6.0557 6.0557 0 0 0 5.7718-4.2058 5.9894 5.9894 0 0 0 3.9977-2.9001 6.0557 6.0557 0 0 0-.7475-7.0729zm-9.022 12.6081a4.4755 4.4755 0 0 1-2.8764-1.0408l.1419-.0804 4.7783-2.7582a.7948.7948 0 0 0 .3927-.6813v-6.7369l2.02 1.1686a.071.071 0 0 1 .038.052v5.5826a4.504 4.504 0 0 1-4.4945 4.4944zm-9.6607-4.1254a4.4708 4.4708 0 0 1-.5346-3.0137l.142.0852 4.783 2.7582a.7712.7712 0 0 0 .7806 0l5.8428-3.3685v2.3324a.0804.0804 0 0 1-.0332.0615L9.74 19.9502a4.4992 4.4992 0 0 1-6.1408-1.6464zM2.3408 7.8956a4.485 4.485 0 0 1 2.3655-1.9728V11.6a.7664.7664 0 0 0 .3879.6765l5.8144 3.3543-2.0201 1.1685a.0757.0757 0 0 1-.071 0l-4.8303-2.7865A4.504 4.504 0 0 1 2.3408 7.872zm16.5963 3.8558L13.1038 8.364 15.1192 7.2a.0757.0757 0 0 1 .071 0l4.8303 2.7913a4.4944 4.4944 0 0 1-.6765 8.1042v-5.6772a.79.79 0 0 0-.407-.667zm2.0107-3.0231l-.142-.0852-4.7735-2.7818a.7759.7759 0 0 0-.7854 0L9.409 9.2297V6.8974a.0662.0662 0 0 1 .0284-.0615l4.8303-2.7866a4.4992 4.4992 0 0 1 6.6802 4.66zM8.3065 12.863l-2.02-1.1638a.0804.0804 0 0 1-.038-.0567V6.0742a4.4992 4.4992 0 0 1 7.3757-3.4537l-.142.0805L8.704 5.459a.7948.7948 0 0 0-.3927.6813zm1.0976-2.3654l2.602-1.4998 2.6069 1.4998v2.9994l-2.5974 1.4997-2.6067-1.4997Z',
  copilot:
    'M23.922 16.997C23.061 18.492 18.063 22.02 12 22.02 5.937 22.02.939 18.492.078 16.997A.641.641 0 0 1 0 16.741v-2.869a.883.883 0 0 1 .053-.22c.372-.935 1.347-2.292 2.605-2.656.167-.429.414-1.055.644-1.517a10.098 10.098 0 0 1-.052-1.086c0-1.331.282-2.499 1.132-3.368.397-.406.89-.717 1.474-.952C7.255 2.937 9.248 1.98 11.978 1.98c2.731 0 4.767.957 6.166 2.093.584.235 1.077.546 1.474.952.85.869 1.132 2.037 1.132 3.368 0 .368-.014.733-.052 1.086.23.462.477 1.088.644 1.517 1.258.364 2.233 1.721 2.605 2.656a.841.841 0 0 1 .053.22v2.869a.641.641 0 0 1-.078.256Zm-11.75-5.992h-.344a4.359 4.359 0 0 1-.355.508c-.77.947-1.918 1.492-3.508 1.492-1.725 0-2.989-.359-3.782-1.259a2.137 2.137 0 0 1-.085-.104L4 11.746v6.585c1.435.779 4.514 2.179 8 2.179 3.486 0 6.565-1.4 8-2.179v-6.585l-.098-.104s-.033.045-.085.104c-.793.9-2.057 1.259-3.782 1.259-1.59 0-2.738-.545-3.508-1.492a4.359 4.359 0 0 1-.355-.508Zm2.328 3.25c.549 0 1 .451 1 1v2c0 .549-.451 1-1 1-.549 0-1-.451-1-1v-2c0-.549.451-1 1-1Zm-5 0c.549 0 1 .451 1 1v2c0 .549-.451 1-1 1-.549 0-1-.451-1-1v-2c0-.549.451-1 1-1Zm3.313-6.185c.136 1.057.403 1.913.878 2.497.442.544 1.134.938 2.344.938 1.573 0 2.292-.337 2.657-.751.384-.435.558-1.15.558-2.361 0-1.14-.243-1.847-.705-2.319-.477-.488-1.319-.862-2.824-1.025-1.487-.161-2.192.138-2.533.529-.269.307-.437.808-.438 1.578v.021c0 .265.021.562.063.893Zm-1.626 0c.042-.331.063-.628.063-.894v-.02c-.001-.77-.169-1.271-.438-1.578-.341-.391-1.046-.69-2.533-.529-1.505.163-2.347.537-2.824 1.025-.462.472-.705 1.179-.705 2.319 0 1.211.175 1.926.558 2.361.365.414 1.084.751 2.657.751 1.21 0 1.902-.394 2.344-.938.475-.584.742-1.44.878-2.497Z',
  gemini:
    'M11.04 19.32Q12 21.51 12 24q0-2.49.93-4.68.96-2.19 2.58-3.81t3.81-2.55Q21.51 12 24 12q-2.49 0-4.68-.93a12.3 12.3 0 0 1-3.81-2.58 12.3 12.3 0 0 1-2.58-3.81Q12 2.49 12 0q0 2.49-.96 4.68-.93 2.19-2.55 3.81a12.3 12.3 0 0 1-3.81 2.58Q2.49 12 0 12q2.49 0 4.68.96 2.19.93 3.81 2.55t2.55 3.81',
  // toolId 'windsurf' → Codeium mark (Windsurf is Codeium's editor)
  windsurf:
    'M1 0a1 1 0 0 0-1 1v22c0 .063.007.124.018.184L0 23.199l.025.026c.103.443.5.775.975.775h22a1 1 0 0 0 1-1V1a1 1 0 0 0-1-1H1zm.707 1h20.582l-2 2H3.707l-2-2zM23 1.705v20.588l-2-2V3.705l2-2zM1 1.707l2 2v16.492l-2 2V1.707zM4 4h16v16H4V4zm3.537 3c-1.006 0-1.51.535-1.51 1.605v2.297c0 .4-.184.6-.554.6a.47.47 0 0 0-.344.139.512.512 0 0 0-.129.365.49.49 0 0 0 .129.353.47.47 0 0 0 .344.139c.37 0 .554.2.554.6v2.297c0 1.07.504 1.605 1.51 1.605.136 0 .248-.05.334-.148A.494.494 0 0 0 8 16.498a.512.512 0 0 0-.129-.365.439.439 0 0 0-.334-.139c-.376 0-.564-.199-.564-.6v-2.296c0-.46-.1-.823-.297-1.092.099-.138.173-.3.222-.485.05-.183.075-.389.075-.619V8.605c0-.4.188-.6.564-.6a.439.439 0 0 0 .334-.138A.499.499 0 0 0 8 7.512a.53.53 0 0 0-.129-.364A.425.425 0 0 0 7.537 7zm8.926 0a.425.425 0 0 0-.334.148.53.53 0 0 0-.129.364.5.5 0 0 0 .129.355.439.439 0 0 0 .334.139c.376 0 .564.199.564.6v2.296c0 .23.025.436.075.62.049.183.123.346.222.484-.197.27-.297.632-.297 1.092v2.297c0 .4-.188.6-.564.6a.439.439 0 0 0-.334.138.512.512 0 0 0-.129.365c0 .145.043.262.129.354a.425.425 0 0 0 .334.148c1.006 0 1.51-.535 1.51-1.605v-2.297c0-.4.184-.6.554-.6a.439.439 0 0 0 .334-.139.475.475 0 0 0 .139-.353.492.492 0 0 0-.139-.365.439.439 0 0 0-.334-.139c-.37 0-.554-.2-.554-.6V8.605c0-1.07-.504-1.605-1.51-1.605zm-7.25 6a.737.737 0 0 0-.496.227.717.717 0 0 0-.217.529.74.74 0 0 0 .75.744.74.74 0 0 0 .75-.744.717.717 0 0 0-.217-.53A.71.71 0 0 0 9.25 13h-.037zm2.75 0a.737.737 0 0 0-.496.227.717.717 0 0 0-.217.529.74.74 0 0 0 .217.53c.152.143.33.214.533.214a.74.74 0 0 0 .75-.744.717.717 0 0 0-.217-.53A.71.71 0 0 0 12 13h-.037zm2.75 0a.737.737 0 0 0-.496.227.717.717 0 0 0-.217.529.74.74 0 0 0 .217.53c.152.143.33.214.533.214a.74.74 0 0 0 .75-.744.717.717 0 0 0-.217-.53.71.71 0 0 0-.533-.226h-.037zm-11.1 8h16.68l2 2H1.613l2-2z',
}

// Round to 3 decimals so generated coordinates stay compact and deterministic.
function r(n: number): string {
  return String(Math.round(n * 1000) / 1000)
}

// Renders a tool icon into a size×size box at (x, y). Brand tools use verbatim
// simple-icons paths; everything else falls back to a rune: a rounded diamond
// (rotated square) in `color` with the tool's uppercase initial in white.
export function renderToolIcon(
  toolId: string,
  x: number,
  y: number,
  size: number,
  color: string,
): string {
  const path = BRAND_PATHS[toolId]
  if (path) {
    const scale = size / 24
    return `<g transform="translate(${r(x)} ${r(y)}) scale(${r(scale)})"><path d="${path}" fill="${color}" /></g>`
  }
  const c = size / 2
  const s = size * 0.72
  const letter = escapeXml((toolId[0] ?? '?').toUpperCase())
  return `<g transform="translate(${r(x)} ${r(y)})">
<rect x="${r(c - s / 2)}" y="${r(c - s / 2)}" width="${r(s)}" height="${r(s)}" rx="${r(s * 0.18)}" fill="${color}" transform="rotate(45 ${r(c)} ${r(c)})" />
<text x="${r(c)}" y="${r(c + size * 0.185)}" font-size="${r(size * 0.5)}" fill="#ffffff" font-weight="bold" text-anchor="middle" font-family="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif">${letter}</text>
</g>`
}

// Hand-drawn element glyph for the archetype-row chip, rendered into a size×size box at
// (x, y). Text/SVG only — no emoji (they rasterize to flat monochrome in GitHub's SVG
// pipeline). Keyed by element id; unknown ids fall back to bolt.
export function renderElementGlyph(
  id: string,
  x: number,
  y: number,
  size: number,
  color: string,
): string {
  const scale = size / 24
  const wrap = (inner: string): string =>
    `<g transform="translate(${r(x)} ${r(y)}) scale(${r(scale)})">${inner}</g>`
  const stroke = `fill="none" stroke="${color}" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"`
  switch (id) {
    case 'lumen': // four-pointed sparkle (synergy)
      return wrap(
        `<path d="M12 1 L13.7 10.3 23 12 13.7 13.7 12 23 10.3 13.7 1 12 10.3 10.3 Z" fill="${color}" />`,
      )
    case 'tide': // two stacked waves (consistency)
      return wrap(`<path d="M3 9 Q6.75 5 10.5 9 T18 9 M3 15 Q6.75 11 10.5 15 T18 15" ${stroke} />`)
    case 'gale': // wind gusts with curled ends (flow)
      return wrap(`<path d="M2 8 H14 Q18 8 16 4.5 M2 13 H18 Q22 13 20 17 M2 18 H11" ${stroke} />`)
    case 'terra': // six-spoke burst + core (range)
      return wrap(
        `<path d="M12 2 V22 M3.3 7 L20.7 17 M3.3 17 L20.7 7" ${stroke} /><circle cx="12" cy="12" r="2.4" fill="${color}" />`,
      )
    case 'blaze': // flame (diversity)
      return wrap(
        `<path d="M12 2 C14 6.5 17 8 16.5 13 A4.5 4.5 0 1 1 7.5 13 C7.5 9.5 10 8.5 10.5 5.5 C11.2 7.5 12 6.5 12 2 Z" fill="${color}" />`,
      )
    default: // bolt — lightning (velocity)
      return wrap(`<path d="M13 2 4 14 11 14 9 22 20 9 13 9 Z" fill="${color}" />`)
  }
}

// Small hand-drawn glyph for each STATS row, rendered into a size×size box at (x, y).
export function renderStatGlyph(
  stat: 'velocity' | 'diversity' | 'consistency',
  x: number,
  y: number,
  size: number,
  color: string,
): string {
  if (stat === 'velocity') {
    // lightning bolt (viewBox 24)
    const scale = size / 24
    return `<g transform="translate(${r(x)} ${r(y)}) scale(${r(scale)})"><path d="M13 2 4 14 11 14 9 22 20 9 13 9 Z" fill="${color}" /></g>`
  }
  if (stat === 'diversity') {
    // three dots in a fan
    const rad = size * 0.13
    return `<g transform="translate(${r(x)} ${r(y)})">
<circle cx="${r(size * 0.5)}" cy="${r(size * 0.26)}" r="${r(rad)}" fill="${color}" />
<circle cx="${r(size * 0.26)}" cy="${r(size * 0.72)}" r="${r(rad)}" fill="${color}" />
<circle cx="${r(size * 0.74)}" cy="${r(size * 0.72)}" r="${r(rad)}" fill="${color}" />
</g>`
  }
  // consistency: a steady wave
  return `<g transform="translate(${r(x)} ${r(y)})">
<path d="M ${r(size * 0.1)} ${r(size * 0.6)} Q ${r(size * 0.3)} ${r(size * 0.2)}, ${r(size * 0.5)} ${r(size * 0.5)} T ${r(size * 0.9)} ${r(size * 0.5)}" fill="none" stroke="${color}" stroke-width="${r(size * 0.14)}" stroke-linecap="round" stroke-linejoin="round" />
</g>`
}
